...
async function analyzeRelevanceToEnvironment(item, userEnv) {
  ...
  // 환경 정보 요약
  const envSummary = `
CLI: ${userEnv.cli_tool.name} (${userEnv.cli_tool.plugins.join(', ')})
MCP 서버: ${Object.keys(userEnv.mcp_servers).join(', ')}
AI 모델: ${Object.keys(userEnv.ai_models).join(', ')}
기술 스택: ${userEnv.technology_stack.join(', ')}
관심사: ${userEnv.interests.join(', ')}
  `.trim();
...
- 벤더: ${vendor}
- 제목: ${title}
- 내용: ${summary.substring(0, 800)}

## 분석 요청
위 릴리스를 분석하여 다음 형식으로 응답해주세요:

**관련도:** [상/중/하/무관]
**분석:** (1-2문장으로 핵심 내용 요약)
**환경 비교:** (사용자 환경과 관련이 있다면 어떤 부분인지, 없다면 이 기술이 무엇인지)
**추천:** [도입 권장 ✅ / 검토 필요 🔍 / 현재 유지 ⏸️ / 해당 없음 ➖]
**사유:** (1문장으로 추천 이유)

응답은 한국어로, 총 5줄 이내로 작성하세요.
`;
...
async function addToNotion(pageId, item, translatedSummary, analysis) {
  const isRss = item.feedType === 'rss';
...
async function main() {
  ...
  for (const item of allItems) {
    ...
    if (notionPageId) {
      try {
        await addToNotion(notionPageId, item, translatedSummary, analysis);
